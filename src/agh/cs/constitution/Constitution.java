...
public class Constitution {
    ...
    public String printArticle(int first, int last) {
        ...
        if (first < 0 ){
            ...
            int tmp = 0;
            while (first > getArticleFromChapter(i,tmp).number)
                tmp++;

            for (int k = i; k <= j; k++) {
                //l to wewnetrzny licznij a nie prawdziwego artykulu
                for (int l = tmp; l < chapters.get(k).articles.size() && getArticleFromChapter(k,l).number <= last; l++) {
                    result=result+getArticleFromChapter(k,l).print();
                }
                tmp = 0;
            }
        }else
    ...
    private Article getArticleFromChapter(int i, int tmp){
        return chapters.get(i).articles.get(tmp);
    }
